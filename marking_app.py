...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # FIX: per-answer Question / Model Answer / Student Answer text is now
        # stored here, keyed by answer_no, so switching between answers in the
        # list never leaves a *previous* answer's Question/Model Answer sitting
        # in the input boxes. Each answer_no gets its own independent draft.
        self.answer_inputs: dict = {}     # {answer_no: {"question":.., "model_answer":.., "student_answer":..}}

        self.worker: MarkingWorker | None = None
        self._current_answer_no: int | None = None
        ...
        # --- extracted text + question + model answer + evaluate ---
        eval_panel = QWidget()
        ...
        # FIX: also save the Question/Model Answer text the user types, into
        # answer_inputs, as they type it — not just at evaluate-time — so
        # navigating away and back preserves what was typed for THAT answer.
        self.question_box.textChanged.connect(self._save_current_inputs)
        self.model_answer_box.textChanged.connect(self._save_current_inputs)
        self.student_answer_box.textChanged.connect(self._save_current_inputs)
    ...
    def _save_current_inputs(self):
        """Persist whatever is currently typed in the three boxes against the
        answer that is currently selected, so it isn't lost / doesn't bleed
        into a different answer when the selection changes."""
        if self._current_answer_no is None:
            return
        self.answer_inputs[self._current_answer_no] = {
            "question": self.question_box.toPlainText(),
            "model_answer": self.model_answer_box.toPlainText(),
            "student_answer": self.student_answer_box.toPlainText(),
        }

    def on_result_selected(self, current, _previous):
        # Save whatever was in the boxes for the answer we're navigating AWAY
        # from, before we overwrite the boxes with the new answer's data.
        ...
        answer_no = index + 1
        self._current_answer_no = answer_no

        saved = self.answer_inputs.get(answer_no)

        # Student answer: prefer a saved edit, else fall back to fresh OCR text.
        if result["status"] == "success":
            student_text = saved["student_answer"] if saved else result["extracted_text"]
            self.student_answer_box.blockSignals(True)
            self.student_answer_box.setPlainText(student_text)
            self.student_answer_box.blockSignals(False)
        ...
        # FIX: Question / Model Answer boxes are now explicitly refilled for
        # THIS answer_no — from saved input if it exists, otherwise cleared.
        # Previously these two boxes were never touched here, so the last
        # answer's Question/Model Answer silently carried over to the next one.
        self.question_box.blockSignals(True)
        self.question_box.setPlainText(saved["question"] if saved else "")
        self.question_box.blockSignals(False)

        self.model_answer_box.blockSignals(True)
        self.model_answer_box.setPlainText(saved["model_answer"] if saved else "")
        self.model_answer_box.blockSignals(False)

        if not saved:
            self.answer_inputs[answer_no] = {
                "question": "",
                "model_answer": "",
                "student_answer": self.student_answer_box.toPlainText(),
            }
    ...
    def on_evaluate_clicked(self):
        ...
        # Make sure the values we're about to grade are also the values saved
        # for this answer_no (belt-and-braces against any stale state).
        self.answer_inputs[answer_no] = {
            "question": question,
