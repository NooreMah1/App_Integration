"""
marking_app.py  (MAIN FILE — isi ko run karna hai: python marking_app.py)
----------------------------------------------------------------------
Colleague ki original GUI + naya panel jisme:
  - Har extracted answer ke liye OCR text (editable)
  - Question + Model Answer input
  - "Evaluate" button -> evaluator_core.evaluate_answer() call hota hai
  - Sab evaluate hone ke baad "Generate Marked PDF" button -> autofiller.py

KEY CONCEPT (yaad rakhna): pdf_extractor.extract_answer_images() jo
image_paths list deta hai, uska ORDER hi global answer_no hai.
List ka pehla item = answer_no 1, dusra = answer_no 2, ... — isi liye
per-page filename ko parse karne ki zaroorat NAHI, bas list index + 1
use karte hain. (Ye pdf_extractor aur autofiller dono ka SAME
box-detection order use karne ki wajah se possible hai.)
"""
...
class MarkingWorker(QThread):
    """Same as colleague's original — box-extract + OCR in background."""
